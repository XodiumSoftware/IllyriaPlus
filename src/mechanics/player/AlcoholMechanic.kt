...
internal object AlcoholMechanic : MechanicInterface {
    ...
    override fun register(): Long {
        AlcoholManager.start()
        return super.register()
    }

    @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
    fun on(event: PlayerItemConsumeEvent) = AlcoholManager.handleDrink(event.player, event.item)

    @EventHandler(priority = EventPriority.MONITOR)
    fun on(event: PlayerJoinEvent) {
        if (event.player.intoxication > 0) {
            AlcoholManager.track(event.player)
            AlcoholManager.applyEffects(event.player)
        }
    }

    @EventHandler(priority = EventPriority.MONITOR)
    fun on(event: PlayerRespawnEvent) {
        AlcoholManager.reset(event.player)
    }

    /** Manages intoxication state, effects, decay, damage, and reset logic. */
    internal object AlcoholManager {
        ...
        /** Starts the periodic decay and damage scheduling tasks. */
        fun start() {
            schedule(period = DECAY_INTERVAL.inWholeSeconds * 20L) {
                trackedPlayers.toList().forEach { applyDecay(it) }
            }
            schedule(period = DAMAGE_INTERVAL.inWholeSeconds * 20L) {
                trackedPlayers.toList().forEach { applyDamage(it) }
            }
        }

        /** Adds a player to the tracked set. */
        ...
        private fun applyDecay(player: Player) {
            if (!player.isOnline || player.lastDrinkTime == 0L) {
                trackedPlayers.remove(player)
                return
            }

            if (System.currentTimeMillis() - player.lastDrinkTime >= RESET_TIMEOUT.inWholeMilliseconds) {
                reset(player)
                return
            }
            ...
            if (player.intoxication == 0) reset(player)
        }
        ...
        private fun applyDamage(player: Player) {
            when {
                ...
                player.intoxication >= DAMAGE_THRESHOLD -> {
