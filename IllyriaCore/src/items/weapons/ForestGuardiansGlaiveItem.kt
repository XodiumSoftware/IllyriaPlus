...
/** Represents a Forest Guardian's Glaive. */
internal object ForestGuardiansGlaiveItem : ItemInterface {
    override val key: NamespacedKey = NamespacedKey(instance, "forest_guardian_glaive")
    ...
    override fun invoke(): ItemStack =
        ItemStack.of(Material.NETHERITE_SPEAR).apply {
            if (!hasData(DataComponentTypes.CUSTOM_NAME) && !hasData(DataComponentTypes.ITEM_NAME)) {
                setData(DataComponentTypes.CUSTOM_NAME, Utils.MM.deserialize("Forest Guardian's Glaive"))
            }
