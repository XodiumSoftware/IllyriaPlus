...
/** Represents a Cybernetic Chainsaw Blade. */
internal object CyberneticChainsawBladeItem : ItemInterface {
    override val key: NamespacedKey = NamespacedKey(instance, "cyberneticsawblade")
    ...
    override fun invoke(): ItemStack =
        ItemStack.of(Material.NETHERITE_SWORD).apply {
            if (!hasData(DataComponentTypes.CUSTOM_NAME) && !hasData(DataComponentTypes.ITEM_NAME)) {
                setData(DataComponentTypes.CUSTOM_NAME, Utils.MM.deserialize("Cybernetic Chainsaw Blade"))
            }
