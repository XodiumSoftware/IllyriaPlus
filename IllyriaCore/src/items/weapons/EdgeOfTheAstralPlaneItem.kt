...
/** Represents a Edge Of The Astral Plane. */
internal object EdgeOfTheAstralPlaneItem : ItemInterface {
    override val key: NamespacedKey = NamespacedKey(instance, "edgeoftheastralplane")
    ...
    override fun invoke(): ItemStack =
        ItemStack.of(Material.NETHERITE_SWORD).apply {
            if (!hasData(DataComponentTypes.CUSTOM_NAME) && !hasData(DataComponentTypes.ITEM_NAME)) {
                setData(DataComponentTypes.CUSTOM_NAME, Utils.MM.deserialize("Edge Of The Astral Plane"))
            }
