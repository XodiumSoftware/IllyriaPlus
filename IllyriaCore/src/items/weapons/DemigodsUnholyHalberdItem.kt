...
/** Represents a Demigod's Unholy Halberd. */
internal object DemigodsUnholyHalberdItem : ItemInterface {
    override val key: NamespacedKey = NamespacedKey(instance, "demigodsunholyhalberd")
    ...
    override fun invoke(): ItemStack =
        ItemStack.of(Material.NETHERITE_SPEAR).apply {
            if (!hasData(DataComponentTypes.CUSTOM_NAME) && !hasData(DataComponentTypes.ITEM_NAME)) {
                setData(DataComponentTypes.CUSTOM_NAME, Utils.MM.deserialize("Demigod's Unholy Halberd"))
            }
