...
/** Represents a Demon Lord's Great Axe. */
internal object DemonLordsGreatAxeItem : ItemInterface {
    override val key: NamespacedKey = NamespacedKey(instance, "demonlordsgreataxe")
    ...
    override fun invoke(): ItemStack =
        ItemStack.of(Material.NETHERITE_SWORD).apply {
            if (!hasData(DataComponentTypes.CUSTOM_NAME) && !hasData(DataComponentTypes.ITEM_NAME)) {
                setData(DataComponentTypes.CUSTOM_NAME, Utils.MM.deserialize("Demon Lord's Great Axe"))
            }
